...
def join_ct_suv(ct: sitk.Image, suv: sitk.Image) -> sitk.Image:
    """
    ...
    res=jnp.stack([jnp.array(suv_arr),jnp.array(ct_arr)],axis=-1)
    return res
...
def load_landmark_data(folder_path:str):
    ...
    arr_0 = join_ct_suv(ct_0, suv_0)
    arr_1 = join_ct_suv(ct_1, suv_1)
...
def get_deauville_1(df, id, st):
    return jnp.array([df.loc[(df['pat_id'] == id) & (df['study_0_or_1'] == st), 'deauville_1'].values[0]])

def get_not_batched(folder_name,img_size):
    # folder_0=load_landmark_data(f"{folder_name}/lin_transf")
    arr_0,arr_1=load_landmark_data(f"{folder_name}/general_transform")

    arr_0=reshape_image(arr_0,img_size)
    arr_1=reshape_image(arr_1,img_size)
    ...
    full_data_table_path="/workspaces/pilot_lymphoma/data/all_deauville_form.csv"
    full_data_table= pd.read_csv(full_data_table_path)
    full_data_table["pat_id"]=full_data_table["pat_id"].astype(int)
    full_data_table["deauville_1"]=(full_data_table["deauville_1"]>3).astype(int)
    ...
    # print(f"pat {get_pat_num_from_path(folder_name)}  0: {get_deauville_1(full_data_table, get_pat_num_from_path(folder_name), 0)}  1: {get_deauville_1(full_data_table, get_pat_num_from_path(folder_name), 1)}  ")
    return [{'study':jnp.expand_dims(arr_0,axis=0), 'deauville':get_deauville_1(full_data_table, get_pat_num_from_path(folder_name), 0)},
            {'study':jnp.expand_dims(arr_1,axis=0), 'deauville':get_deauville_1(full_data_table, get_pat_num_from_path(folder_name), 1)}
            ]
...
def get_dataset(folder_path,img_size):
    ...
    res= [get_not_batched(folder_name,img_size) for folder_name in folder_names]
